...
```{r Bar plot - count number of genes per chromosome, echo=TRUE}
...
```{r shapiro test - U distribution}

# Testing normality (U distribution)

u_dist = runif(1000, min = 2, max = 4)
shapiro.test(u_dist)
hist(u_dist)
...
```{r shapiro test - N distribution}

# Testing normality (N distribution)

n_dist = rnorm(1000, mean = 5, sd = 3)
shapiro.test(n_dist)
hist(n_dist)
