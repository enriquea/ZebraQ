...
## Capstone: what are these chambers doing?

Everything up to here has been guided. This part is not.

You have a human fetal heart dataset, four chambers, and the workflow from S6.
This data is already human, so no ortholog mapping is needed — go straight to
<https://maayanlab.cloud/Enrichr/> and use **GO Biological Process 2025**.

### Step 1 — two lists, deliberately different
...
# List B: the genes most specific to the left atrium relative to the right.
list_B <- atrial %>%
...
# How much do they even have in common?
...
**Predict before you look.** Write down what you expect each list to return.

### Step 3 — the question

One of these lists will come back full of ribosomes, mitochondria and muscle
contraction. The other will come back with terms that actually distinguish one
chamber from another.

- Which is which, and why?
- The top-expressed list of *any* human tissue returns roughly the same answer.
  What does that tell you about using "most expressed" as a gene list?
- What is the implicit background set in each case, and is it the right one?

### Step 4 — apply the S6 rule

For whichever list gave you the more interesting answer, open the `Genes`
column of the top five terms.

- Is any term driven by a single gene family?
- Do two or three terms rest on the same genes under different names?
- Is any term obviously mislabelled for this tissue?

Report the two or three findings you would actually defend, and say plainly
which ones you discarded and why. That last sentence is the part that makes it
science.

### Step 5 — if you have time

Repeat for the ventricles using `ventricular` and `delta_LV_RV`. Atria and
ventricles are far more different from each other than left is from right. Does
the enrichment show that?
